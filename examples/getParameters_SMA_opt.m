...
% =============================================================================
    ...
    if iter == 1
        ...
        opt.KA = [0, 1.59, 7.70, 35.5]; % [MB, BSA], A for raffinate, B for extract
        ...
        flowRate.recycle   = 2.21e-8;      % m^3/s
        flowRate.desorbent = 1.14e-8;      % m^3/s
        flowRate.extract   = 1.09e-8;      % m^3/s
        flowRate.feed      = 0.55e-8;      % m^3/s
        flowRate.raffinate = 0.60e-8;      % m^3/s
        ...
        if strcmp('StericMassActionBinding', opt.BindingModel)
            opt.concentrationSalt  = [290, 420]; % mol/m^3
    ...
    elseif iter == 2
        ...
        opt.nComponents = 2;
        opt.LAMBDA = 1.2e3; % mol/m^3
        opt.KA = [0, 1.59, 7.70]; % [MB, BSA], A for raffinate, B for extract
        opt.KD = [0, 1000, 1000];
        opt.NU = [1, 5.29, 3.70];
        ...
        concentrationFeed   = [0.1, 0.1];   % g/m^3
