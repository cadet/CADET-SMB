...
% =============================================================================
    ...
    if iter == 1
        ...
        opt.filmDiffusion             = [5e-5 5e-5 5e-5];
        opt.diffusionParticle         = [1.6e4 1.6e4 1.6e4];
    ...
    elseif iter == 2
        ...
        opt.BindingModel = 'LinearBinding';
        opt.nComponents = 2;
        opt.KA = [0.28, 0.61]; % [comp_A, comp_B], A for raffinate, B for extract
        opt.KD = [1, 1];
        opt.compTargID = [2, 1];

        % Transport
        opt.dispersionColumn          = ones(1,opt.nZone) .* 3.8148e-6;
        opt.filmDiffusion             = [5e-5 5e-5];
        opt.diffusionParticle         = [1.6e4 1.6e4];
